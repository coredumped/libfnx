...
namespace fnx {
	...
	MutexLockException::MutexLockException(const std::string &_errmsg) : fnx::GenericException(_errmsg){
		
	}
	
	MutexLockException::MutexLockException(const std::string &_errmsg, const char *_srcFile, int _srcLine) : fnx::GenericException(_errmsg){
		srcFile = _srcFile;
		srcLine = _srcLine;
	}
